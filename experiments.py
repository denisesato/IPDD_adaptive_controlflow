...
def dataset1_new_strategie():
    ...
    output_folder = f'data/output/controlflow_adaptive/new_strategie_detection_2metrics_updating_model'
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    drifts = dict.fromkeys(lognames)
    for log in lognames:
        drifts[log] = {}
        for d in deltas:
            drifts[log][f'd={d}'] = apply_adwin_updating_model_after_each_trace(input_folder, log, d, output_folder)

    df1 = pd.DataFrame.from_dict(drifts, orient='index')
    df1.to_excel(os.path.join(output_folder, 'experiments_new_strategie_dataset1.xlsx'))


if __name__ == '__main__':
    # dataset1()
    # dataset1_new_strategie()
